...
logicType τ      = fromRTypeRep $ t { ty_binds = bs, ty_args = as, ty_refts = rs}
  where
    ...
    (bs, as, rs) = unzip3 $ dropWhile (isErasableType . Misc.snd3) $ zip3 (ty_binds t) (ty_args t) (ty_refts t)
...
inlineSpecType v = fromRTypeRep $ rep {ty_res = res `strengthen` r , ty_binds = xs}
  where
    ...
    vxs            = dropWhile (isErasableType . snd) $ zip xs (ty_args rep)
    f              = dummyLoc (symbol v)
...
weakenResult v t = F.notracepp msg t'
  where 
    ...
    vE           = mkEApp vF xs
    xs           = EVar . fst <$> dropWhile (isErasableType . snd) xts 
...
isErasable :: Id -> Bool
isErasable v = F.notracepp msg $ isGhcSplId v && not (isDCId v)  && not (GM.isDictionary v)
  where 
