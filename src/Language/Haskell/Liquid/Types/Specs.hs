...
data GhcSpec = SP 
  ...
  , gsRefl   :: !GhcSpecRefl   
  , gsLaws   :: !GhcSpecLaws 
  , gsImps   :: ![(F.FixSymbol, F.Sort)]  -- ^ Imported Environment          
  , gsConfig :: !Config                       
...
data GhcSpecData = SpData 
  ...
  , gsMeas       :: ![(LHSymbol, LocSpecType)]    -- ^ Measure Types eg.  len :: [a] -> Int
...
data LawInstance = LawInstance
  ...
  -- YL: VarOrLocSymbol -> LHSymbol : is this a good idea? 
  , lilEqus   :: [(LHSymbol, (LHSymbol, Maybe LocSpecType))]
  , lilPos    :: SrcSpan
  }  
...
data Spec ty bndr  = Spec
  ...
  , expSigs    :: ![(LHSymbol, F.Sort)]           -- ^ Exported variables types
  ...
  , ealiases   :: ![F.Located (RTAlias F.FixSymbol F.Expr)]   -- ^ Expression aliases
  , embeds     :: !(F.TCEmb FixSymbol F.LocSymbol)                   -- ^ GHC-Tycon-to-fixpoint Tycon map
