...
isClassType (RApp c _ _ _) = isNumCls c || isEqual c
isClassType _              = False
...
instance TyConable TyCon where
  ...
  isClass c  = isClassTyCon c   || isEqual c
...
efoldReft logicBind cb dty g f fp = go
  where
    ...
    go γ z me@(RFun _ (RApp c ts _ _) t' r)
       | isNumCls c || isEqual c        = f γ (Just me) r (go (insertsSEnv γ (cb c ts)) (go' γ z ts) t')
